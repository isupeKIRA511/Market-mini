...
<nav class="absolute bottom-0 left-0 w-full z-50 flex flex-row-reverse justify-around items-center px-1 pb-6 pt-3 bg-[#1D1B1C] sm:rounded-b-[32px] rounded-t-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.15)] overflow-x-hidden">
  {#each navItems as item}
    ...
    <div 
        ...
        {isItemActive(item.id, $currentRoute) ? 'bg-[#FAC445] text-[#1D1B1C]' : 'text-white/70 hover:text-white'}"
    >
      <div class="mb-1">
          {#if item.id === 'home'}
